...
class LorentzParTConfig(BaseModelConfig):
    ...
    num_extra_features: int = 0        # per-particle scalars beyond the 4-vector (Phase 6: 4 disp + 6 PID)
...
class JEPAConfig(BaseModelConfig):
    ...
    def from_dict(cls, d: Dict):
        ...
        if 'jepa' in flat:
            ...
            flat.setdefault('num_mask', jepa.get('num_mask', 1))
            flat.setdefault('mask_mode', jepa.get('mask_mode', 'random'))
