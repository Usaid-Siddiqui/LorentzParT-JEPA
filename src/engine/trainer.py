...
class Trainer:
    ...
    def __init__(
        ...
        if config is not None:
            ...
            _sched_cfg = scheduler if scheduler is not None else config.scheduler
            self.scheduler = get_scheduler_from_config(_sched_cfg, SCHEDULER_REGISTRY, self.optimizer) if _sched_cfg is not None else None
            _cb_cfg = callbacks if callbacks is not None else config.callbacks
            self.callbacks = get_callbacks_from_config(_cb_cfg, CALLBACK_REGISTRY) if _cb_cfg is not None else []
