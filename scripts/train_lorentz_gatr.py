"""
LorentzGATr training script — supervised classification on JetClass.

Trains the full L-GATr backbone for jet classification, optionally loading
pretrained encoder weights. Use as a baseline to compare against LorentzParT
and isolate the contribution of deep per-block Lorentz equivariance.

Usage:
    # From scratch
    python scripts/train_lorentz_gatr.py \\
        --data-dir ./data \\
        --run-name lorentz_gatr_seed42 \\
        --seed 42

    # From pretrained weights
    python scripts/train_lorentz_gatr.py \\
        --data-dir ./data \\
        --weights ./logs/LorentzGATr/best/<run>.pt \\
        --run-name lorentz_gatr_ft_seed42 \\
        --seed 42

Trained models are saved to ./logs/LorentzGATr/best/<run-name>.pt
"""
...
from src.configs import LGATrConfig, TrainConfig
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train LorentzGATr for jet classification")
    parser.add_argument('--data-dir',       type=str, default='./data')
    parser.add_argument('--config-path',    type=str, default='./configs/train_lorentz_gatr.yaml')
    parser.add_argument('--weights',        type=str, default=None,
                        help="Path to pretrained encoder weights (.pt)")
...
def main(rank, world_size, seed, config_path, data_dir, weights, run_name, checkpoint_path=None):
    ...
    model_cfg = LGATrConfig.from_dict(config['model'])
    ...
    model = LorentzGATr(config=model_cfg).to(device)
...
if __name__ == '__main__':
    ...
    if world_size > 1:
        ...
        main(
            rank=0, world_size=1,
            seed=args.seed, config_path=args.config_path,
            data_dir=args.data_dir, weights=args.weights,
            run_name=args.run_name, checkpoint_path=args.checkpoint_path,
        )
