"""
ParticleTransformer training script — supervised classification on JetClass.

Trains the vanilla ParticleTransformer (ParT-style pairwise interaction
attention, no Lorentz-equivariant layers). Use as a baseline to compare
against LorentzParT and isolate the contribution of Lorentz equivariance.

Usage:
    # From scratch
    python scripts/train_particle_transformer.py \\
        --data-dir ./data \\
        --run-name particle_transformer_seed42 \\
        --seed 42

    # From pretrained weights
    python scripts/train_particle_transformer.py \\
        --data-dir ./data \\
        --weights ./logs/ParticleTransformer/best/<run>.pt \\
        --run-name particle_transformer_ft_seed42 \\
        --seed 42

Trained models are saved to ./logs/ParticleTransformer/best/<run-name>.pt
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train ParticleTransformer for jet classification")
    parser.add_argument('--data-dir',        type=str, default='./data')
    parser.add_argument('--config-path',     type=str, default='./configs/train_particle_transformer.yaml')
...
def main(rank, world_size, seed, config_path, data_dir, weights, run_name, checkpoint_path=None):
    ...
    model_cfg = ParticleTransformerConfig.from_dict(config['model'])
